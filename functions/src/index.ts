...
export const migrateUserSetListOrders = functions
  .https.onRequest(async (request, response) => {
    if (request.query.secret !== MIGRATION_SECRET_KEY) {
      functions.logger.error(
        "Unauthorized attempt to run setlist order migration.Invalid secret.");
      response.status(403).send("Unauthorized: Invalid secret key.");
      return;
    }

    functions.logger.info(
      "Starting migration for all user setlist orders (triggered via HTTPS)");
    ...
    try {
      ...
      const migrationPromises = usersSnapshot.docs.map(async (userDoc) => {
        ...
        setListsSnapshot.docs.forEach((slDoc, index) => {
          ...
          if (slData.order === undefined || typeof
          slData.order !== "number" || slData.order !== index) {
            functions.logger.info(`User ${userId}, SetList ${
              slDoc.id} (Title: ${slData.title || "N/A"

            }):setting order to ${index}`);
        ...
        if (userSetListsUpdated > 0) {
          await batch.commit();
          functions.logger.info(`Committed ${
            userSetListsUpdated} setlist order updates for user ${userId}.`);
          totalSetListsMigrated += userSetListsUpdated;
        } else {
          functions.logger.info(
            `No setlist order updates needed for user ${userId}.`);
        }
      });

      await Promise.all(migrationPromises);

      const successMessage = `Setlist order migration completed. 
      Total setlists updated: ${totalSetListsMigrated}.`;
